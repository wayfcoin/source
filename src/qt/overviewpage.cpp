...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeWayfcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeWayfcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeWayfcoinAmount = strAnonymizeWayfcoinAmount.remove(strAnonymizeWayfcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWayfcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeWayfcoinAmount*COIN) nMaxToAnonymize = nAnonymizeWayfcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeWayfcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeWayfcoinAmount));
        strAnonymizeWayfcoinAmount = strAnonymizeWayfcoinAmount.remove(strAnonymizeWayfcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWayfcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
