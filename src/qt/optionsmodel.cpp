...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeWayfcoinAmount"))
        settings.setValue("nAnonymizeWayfcoinAmount", 1000);
    nAnonymizeWayfcoinAmount = settings.value("nAnonymizeWayfcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeWayfcoinAmount"))
        SoftSetArg("-anonymizeWayfcoinamount", settings.value("nAnonymizeWayfcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeWayfcoinAmount:
            return QVariant(nAnonymizeWayfcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeWayfcoinAmount:
            nAnonymizeWayfcoinAmount = value.toInt();
            settings.setValue("nAnonymizeWayfcoinAmount", nAnonymizeWayfcoinAmount);
            emit AnonymizeWayfcoinAmountChanged(nAnonymizeWayfcoinAmount);
            break;
        default:
            break;
        }
