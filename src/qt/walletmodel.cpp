...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CWayfcoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CWayfcoinAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject wayfcoin fee
        unsigned int wayfcoinfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        wayfcoinfee = ((MIN_RELAY_TX_FEE * wayfcoinfee) * 10000);
        if (nFeeRequired > wayfcoinfee){
            LogPrintf("nFeeRequired: %d -- InsaneFee: %d\n", nFeeRequired, wayfcoinfee);
            return InsaneFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CWayfcoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CWayfcoinAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CWayfcoinAddress(address).ToString());
...
{
    // Wayfcoin context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CWayfcoinAddress(address).ToString())].push_back(out);
    }
}
